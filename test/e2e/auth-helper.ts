...
const DEFAULT_TOKEN_AUTH_METHOD: ClientAuthMethod = 'none';
...
export async function acquireOidcTokens(overrideEmail?: string): Promise<OidcTokens> {
  ...
  const email = overrideEmail ?? readEnv('E2E_OIDC_EMAIL', DEFAULT_EMAIL);
  ...
  if (!['client_secret_basic', 'client_secret_post', 'none'].includes(authMethodValue)) {
...
export async function acquireAccessToken(email?: string): Promise<AccessTokenResult> {
  const tokens = await acquireOidcTokens(email);
  const user = JSON.parse(tokens.userJson) as {
    access_token: string;
    profile: Record<string, unknown>;
  };
  const sub = user.profile.sub;
  const profileEmail = user.profile.email;
  if (typeof sub !== 'string' || typeof user.access_token !== 'string') {
    throw new Error('Failed to extract sub or access_token from OIDC response');
  }
  if (typeof profileEmail !== 'string') {
    throw new Error('Failed to extract email from OIDC profile');
  }
  return { accessToken: user.access_token, sub, email: profileEmail };
}
